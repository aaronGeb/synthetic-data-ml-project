...
class ProductListing(BaseModel):
    product_id: int = Field(
        ..., description="Unique identifier for the product", example=1234
    )
    category: str = Field(
        ...,
        description="Product category (Electronics, Clothing, or Furniture)",
        example="Electronics",
    )
    brand: str = Field(..., description="Brand name", example="Samsung")
    condition: str = Field(
        ..., description="Product condition (New or Used)", example="New"
    )
    price: float = Field(..., description="Product price in USD", gt=0, example=1200.00)
    seller_reputation: int = Field(
        ..., description="Seller reputation score (1-5)", ge=1, le=5, example=4
    )
    description: str = Field(
        ...,
        description="Detailed product description",
        example="Brand new Samsung Galaxy S21 Ultra with 256GB storage",
    )
    suspected_fraud: bool = Field(
        ..., description="Whether the listing is suspected of fraud", example=False
    )
